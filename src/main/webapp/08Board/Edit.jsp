...
<%
/* 수정은 기본적으로 내용보기와 로직이 동일하다. 기존의 내용을 인출한후
작성폼에 설정해야 하기 때문이다. 
파라미터를 받은 후 수정된 게시물을 인출한다.*/
String num = request.getParameter("num");
BoardDAO dao = new BoardDAO(application);
BoardDTO dto = dao.selectView(num);
/*
로그인 한 회원이 해당 게시물의 작성자인지 확인한다.
세션영역에 저장된 회원 아이디를 가져와서 문자열로 변경한 후
...
<!-- 
게시물의 일련번호를 서버로 전송하기 위햬 hidden 타입의 입력상자를
반드시 추가해야 한다. 이 부분이 추가되지 않으면 게시물은 수정되지 않는다.
-->
