...
<body>
	...
	// 개발자가 직접 정의한 클래스는 원래의 타입으로 형변환 한다.
	...
	*/
	%>
	<ul>
		<!-- Java의 기본 클래스는 별도의 처리 없이 즉시 내용을 출력할 수 있다. -->
		<li>Integer 객체 : <%= pInteger %></li>
		<li>String 객체 : <%= pString %></li>		 
		<!-- Person 클래스는  getter를 통해 내용을 출력한다. -->
		<li>Person.getName() 객체 : <%= pPerson.getName() %>, <%= pPerson.getAge()%></li>
	</ul>
	
	<!-- 
	지시어를 통한 include는 포함시킬 페이지의 원본소스를 그대로 현재 문서에
	포함시킨 후 컴파일을 진행한다. 따라서 동일한 페이지로 취급하므로
	page 영역이 그대로 유지된다.
