...
<body>
    <h3>변수 선언 및 할당</h3>
    <!-- 
    EL 및 JSTL 영역에 저장된 속성을 이용해서 연산하거나 출력한다.
    따라서 스크립트렛에서 선언한 변수는 EL에서 사용할 수 없다.
    따라서 num1은 아무값도 출력되지 않는다.
     -->
    스크립트렛에서 선언한 변수 : ${ num1 }  <br />    
    page 영역에 저장된 변수 : ${ num2 } <br />
    
    <!-- 
    Tomcat 8.0부터는 EL에서 변수할당을 할 수 있다. 하지만 할당과 동시에
    ...
    <!-- 
    나눗셈에서는 slash(/)와 % 대신 div, mod를 사용할 수 있다.
    또한 자동 형변환이 적용되므로 정수끼리의 연산에서 실수의 결과를
    반환할 수 있다.
     -->
    ...
    적용한 것과 동일한 결과가 반환된다.
    위에서 num1에 7을 할당하였으므로 41이 출력된다.  -->
    num1 + "34" : ${ num1 + "34" } <br />
    <!-- 하지만 숫자와 문자열, 혹은 문자열과 문자열의 경우에는 덧셈 연산이
    불가능하므로 예외(NumberFormatException)가 발생한다. -->
    num2 + "이십" : \${num2 + "이십" }  <br /> 
    "삼십" + "사십" : \${"삼십" + "사십" } 
    ...
    <!-- 
    논리AND, 논리OR 연산자 사용시 &&, || 대신 알파벳 형식의 연산자(gt등)를
    사용할 수 있다.
     -->
    <h3>논리 연산자</h3>
