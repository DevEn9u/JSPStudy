...
// String 타입의 폼값을 날짜 형식으로 변경한 후 타임스탬프를 얻어온다.
long add_date = s.parse(request.getParameter("add_date")).getTime();

// 숫자형식으로 전송된 폼값을 정수로 변환한다.
...
addDateHeader(헤더명, long타입의 타임스탬프)
	: 응답헤더에 날짜형식을 추가하는 경우 long 타입의 타임스탬프로 
	변환 후 추가하면 된다.
addIntHeader()
	: 숫자형식의 응답헤더를 추가한다.
addHeader()
...
<body>
	...
	for (String hName : headerNames) {
		// getHeaders() 메서드로 헤더명을 통해 헤더값을 얻어온 후 출력한다.
	...
	*/
	...
	<%
	/*
	myNumber라는 헤더명으로 2개의 값을 추가하였으므로 아래에서는 각각의
	값이 정상적으로 출력된다. 즉 add 계열의 메서드(getHeaders)는 헤더명을 동일하게
